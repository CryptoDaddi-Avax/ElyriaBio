...
(function(){
  ...
  /* ---------- size-variant tabs (.stier) — delegated on scope ---------- */
  // The page's inline script clones #strengthTiers on DOMContentLoaded (after defer scripts run),
  // so direct listeners on .stier get orphaned. Use event delegation on scope instead.
  function updateStockForStier(st){
  ...
  // Delegated listener — fires for clicks on any .stier inside scope
  scope.addEventListener("click", function(e){
    var el = e.target;
    while(el && el !== scope){
      if(el.classList && el.classList.contains("stier")){ updateStockForStier(el); return; }
      el = el.parentElement;
    }
  });
